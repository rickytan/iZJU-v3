...
+ (NSString*)getSSID
{
    ...
    __autoreleasing NSString *ssid = [(__bridge NSDictionary*)info valueForKey:(NSString*)kCNNetworkInfoKeySSID];
    if (info)
        CFRelease(info);
    if (ifs)
        CFRelease((__bridge CFTypeRef)(ifs));
...
- (void)onLogout:(id)sender
{
    ...
    [logoutRequest setCompletionBlock:^{
        if (![r.responseString isEqualToString:@"logout_ok"]) {
            [weakSelf.view makeToast:r.responseString];
            weakSelf.navigationItem.rightBarButtonItem.enabled = YES;
        }
        else
            [weakSelf networkHandler:nil];
    }];
...
- (void)kickOut
{
    ...
    [kickOutRequest setCompletionBlock:^{
        if ([r.responseString isEqualToString:@"ok"])
            [weakSelf authIfNeeded];
        else {
            [weakSelf.view makeToast:[NSString stringWithFormat:@"踢除失败:%@",r.responseString]];
    ...
    switch (indexPath.section) {
        ...
        case 1:
            switch (indexPath.row) {
                ...
                case 1:
                {
                    UIViewController *helpController = [[ZJUBaseViewController alloc] init];
